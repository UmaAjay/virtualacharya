...
def ui_api_key():
	if ss['community_user']:
		...
		api_key='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'
		st.text_input('"Enter your email 👇"', type='default', key='api_key', on_change=on_api_key_change, label_visibility="collapsed")
...
def b_save():
	...
	if st.button('save encrypted index in virtualacharya using nlp', help=help):
		with st.spinner('saving to VirtualAcharya using OpenaAi, NLP'):
			db.put(name, index)
